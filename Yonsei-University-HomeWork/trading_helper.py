...
TRADING_HELPER_VERSION = '2022.11.30-v1'
...
#
# ticker - 암호화폐 티커              加密货币计票器
# date1 - 시작 날짜 YYYYMMDD 형식     开始日期YYYYMMDD格式
# date2 - 종료 날짜 YYYYMMDD 형식     结束日期YYYYMMDD格式
#
def get_ohlcv2(ticker, date1, date2):

    try:

        df = pd.read_csv(f'https://esohn.be/semester/2022-2/dfs/{ticker}_{date1}_{date2}.csv')
...
def get_multiple(date1, date2):
    ...
    try:
        ...
        df = pd.read_csv(f'https://esohn.be/semester/2022-2/dfs/multiple_{date1}_{date2}.csv')
